...
!!! tip "🔬 方法选择指南：轨迹分析方法全景对比"

    | 方法 | 输入 | 检测分支 | 方向性 | 速度 | 最佳场景 |
    |---|---|---|---|---|---|
    | PAGA ✓ | KNN图 + 聚类 | 拓扑连接图 | 无 | ⚡ 快 | 全局概览（推荐首选） |
    | DPT ✓ | 扩散图 | 需预设root | 伪时间排序 | ⚡ 快 | 线性/简单分化轨迹 |
    | Monocle 3 | UMAP + 图学习 | 自动检测 | 伪时间排序 | 中 | 复杂分支轨迹（R生态） |
    | Slingshot | 降维坐标 + MST | 自动检测 | 伪时间排序 | 快 | 简单分支（R生态） |
    | scVelo | 剪接/未剪接RNA比值 | 方向推断 | RNA velocity向量 | 中 | 需要剪接信息的方向推断 |
    | Palantir | 扩散图 + 马尔可夫链 | 概率性分支 | 分化概率 | 中 | 概率性分支+多终点 |
    | CytoTRACE2 ✓ | 表达矩阵 | 否 | 发育潜能评分 | 中 | 评估干性/分化状态 |
